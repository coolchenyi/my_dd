...
import io.druid.server.yeahmobi.ResultDbSaver;
...
public class QueryResource
{
  ...
  ) throws ServletException, IOException
  {
    ...
    try {
      ...
      // modify the druid origin logic to implement the special function
      // although this solution is ugly, but have no other solution
      // use in-memory DB to do the secondary group operation
      if(ResultDbSaver.isNeedSaveToDb(req, query)){
    	  ResultDbSaver dataSaver = new ResultDbSaver(req, results, objectMapper, jsonWriter, query);
    	  // insert the druid data into in-memory DB
    	  dataSaver.insertData();
    	  
    	  // needs special process, handle landing page
    	  String msg = String.format("insert into table %s successfully", dataSaver.getTableName());
